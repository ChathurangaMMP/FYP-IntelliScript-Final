...
query_prompt_qna = """Generate an accurate Output for the given Query only considering the given Context. Make the output shorter and summarized. Do not add lengthy explanations in the output."""
...
Make the output accurate, short, and summarized. Do not generate lengthy explanations.
...
def generate_final_report(query, temp, context_len):
    ...
    if topics_list != []:
        ...
        with open('report-metadata.txt', 'w') as report_mt:
            report_mt.write(str(metadata))
